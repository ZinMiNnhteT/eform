        ...
        <div class="card">
            ...
            <div class="card-body">
                    ...
                    if ($files->count() > 0){
                        foreach ($files as $file){
                            if ($file->city_license){
                                $data = explode(',', $file->city_license);
                ...
                @if($required == 'required')
                    ...
                    <br/>
                @endif
                {!! Form::open(['route' => 'resident_power_city_license_update_mdy', 'method' => 'PATCH', 'files' => true]) !!}
                {!! Form::hidden('form_id', $form_id, ['id' => 'form_id']) !!}
                    <div class="row">
                        <div class="col-12">
                            <h4 class="card-title ">{{ __('lang.city_license') }} {!! $star !!}</h4>
                    ...
                    <div class="row m-t-10 m-b-10">
                        @foreach ($files as $file)
                            @if ($file->city_license)
                        @php $data = explode(',', $file->city_license); @endphp
                                @foreach ($data as $item)
            ...
            <div class="card-footer text-center">
                ...
                <a href="{{ route('resident_applied_form_mdy', $form->id) }}" class="waves-effect waves-light btn btn-secondary btn-rounded col-md-3">{{ __('lang.cancel') }}</a>
