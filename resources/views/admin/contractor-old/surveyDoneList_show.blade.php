        ...
        <div class="card">
            ...
            <div class="card-body card-body-pb-div">
                ...
                <div class="row justify-content-center">
                    <div class="col-md-4"> 
                        @if (chk_c_form($data->id)['send_from'] == 1 && chk_c_form($data->id)['send_to'] == 2)
                            @if (hasPermissionsAndGroupLvl(['residentialChkGrdDone-create'], admin()->group_lvl)) {{--  if login-user is from township  --}}
                                {{-- @if(Auth::user()->hasRole('SeniorEngineer(Township)'))  --}}
                        <a href="{{ route('contractorMeterGroundCheckDoneList.createdist', $data->id) }}" class="btn btn-block btn-rounded btn-warning" data-toggle="tooltip" data-title="To Confirm Survey Result">
                            {{ __('lang.confirm_survey') }}
                        </a>
                                {{-- @endif  --}}
                            @endif
                        @elseif ((chk_c_form($data->id)['send_from'] == 1 && chk_c_form($data->id)['send_to'] == 3) || (chk_c_form($data->id)['send_from'] == 2 && chk_c_form($data->id)['send_to'] == 3))
                            @if (hasPermissionsAndGroupLvlForContractor(['residentialChkGrdDone-create'], admin()->group_lvl)) {{--  if login-user is from township  --}}
                                {{-- @if(Auth::user()->hasRole('SeniorEngineer(Township)'))  --}}
                        <a href="{{ route('contractorMeterGroundCheckDoneList.createdivstate', $data->id) }}" class="btn btn-warning" data-toggle="tooltip" data-title="To Confirm Survey Result">
                            {{ __('lang.confirm_survey') }}
                        </a>
                                {{-- @endif  --}}
                            @endif
