    ...
    <div class="col-12">
        <div class="card">
            <div class="card-header text-white bg-info d-flex">
                ...
                <div class="ml-auto">
                    <a href="{{ route('residentialPowerMeterPendingForm.index') }}" class="btn btn-rounded btn-info text-white l-h-ini p-t-5 p-b-5"><i class="fa-fw fa fa-angle-double-left"></i>{{ __('lang.back') }}</a>
                </div>
            </div>
                ...
                <div class="container-fluid">
                    ...
                    <div class="card mb-1">
                        @if (chk_userForm($data->id)['pending'])
                            @if (hasPermissions(['residentPowerPending-create'])) {{--  if login-user is from township  --}}
                        <div class="card-header d-flex" role="tab" id="headingOne">
                            <h5 class="mb-0 text-info">{{ __('စောင့်ဆိုင်းစာရင်းမှ ပယ်ဖျက်ရန်') }}</h5>
                        </div>
                        <div class="card-body">
                            {!! Form::open(['route' => 'residentialPowerMeterPendingForm.store', 'files' => true]) !!}
